...
namespace BookRentalShopApp20.Subitems
{
    public partial class MemberMngform : MetroForm
    {
        ...
        private void UpdateComboDivision()  //콤보박스 데이터 넣는 함수
        {
            using (MySqlConnection conn = new MySqlConnection(Commons.CONNSTR))
            {
                string strQuery = $"SELECT DISTINCT Levels " +
                                   "FROM membertbl " +
                                   "ORDER BY Levels ";
        ...
        private void SaveData()
        {
            ...
            if (string.IsNullOrEmpty(TxtNames.Text) || CboLevels.SelectedIndex < 1 || string.IsNullOrEmpty(TxtAddr.Text) || string.IsNullOrEmpty(TxtEmail.Text))
            {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Commons.CONNSTR))
                {
                    ...
                    {
                        Value = CboLevels.SelectedValue   //cbo는 selectedvalue사용
                    };
        ...
        private void GrdDivTbl_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > -1)
            {
                ...
                CboLevels.SelectedValue = data.Cells[2].Value.ToString();
